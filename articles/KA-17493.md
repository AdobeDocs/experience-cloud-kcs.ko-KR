---
title: Adobe Managed Services Dispatcher 플러싱
description: 설명
...
<b>문제/증상</b>
이 문서는 플러시 발생 방법에 대한 지침을 제공하고 캐시 플러시 및 무효화를 실행하는 메커니즘에 대해 설명합니다.


<br>작동 방식<br>작업 순서<br>
일반적인 워크플로우는 콘텐츠 작성자가 페이지를 활성화할 때 가장 잘 설명되며 게시자가 새 콘텐츠를 받으면 다음 다이어그램과 같이 디스패처에 대한 플러시 요청을 트리거합니다.
![작성자가 콘텐츠를 활성화하면 게시자가 디스패처에 플러시 요청을 보내도록 트리거합니다.](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/dispatcher-flushing-order-of-events.png "dispatcher 플러시 이벤트 순서")
...
![AEM 웹 페이지 /etc/replication.html의 표준 복제 에이전트의 스크린샷](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/author-rep-agent-example.png "author-rep-agent-example")
...
![AEM 웹 페이지 /etc/replication.html의 표준 플러시 복제 에이전트의 스크린샷](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")<br>DISPATCHER 플러시 복제 수신 가상 호스트<br>
...
![Dispatcher 플러시로 표시된 직렬화 유형의 기본 구성 화면 설정 탭 그림](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent1.png "disp-flush-agent1")
기본 설정 페이지에는 <b>직렬화 유형</b> 다음으로: *Dispatcher 초기화* 오류 수준을 설정합니다.
![복제 에이전트의 전송 탭 스크린샷입니다.  플러시 요청을 게시할 URI를 표시합니다.  /dispatcher/invalidate.cache](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent2.png "disp-flush-agent2")
전송 탭에는 플러시 요청을 수신할 디스패처의 IP 주소를 가리키도록 설정되는 URI를 볼 수 있습니다.  경로 /dispatcher/invalidate.cache 는 플러시인지 여부를 모듈에서 결정하는 방법이 아닙니다. 플러시 요청임을 알기 위해 액세스 로그에서 볼 수 있는 명백한 종단점입니다.  확장 탭에서 디스패처 모듈에 대한 플러시 요청임을 확인하기 위한 항목을 살펴봅니다.
![복제 에이전트의 확장 탭 스크린샷입니다.  디스패처에게 플러시하도록 알리기 위해 POST 요청과 함께 전송된 헤더를 참고하십시오.](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent3.png "disp-flush-agent3")
...
- Host
   - Dispatcher Apache 웹 서버(/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost)에 구성된 특정 VirtualHost를 대상으로 호스트 헤더가 스푸핑됩니다.  aem_flush.vhost 파일의 항목과 일치하는 하드 코딩된 값입니다. *서버 이름* 또는 *ServerAlias*

![작성자 게시 컨텐츠의 복제 이벤트에서 새 항목이 수신될 때 복제 에이전트가 반응 및 트리거됨을 보여주는 표준 복제 에이전트의 화면](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent4.png "disp-flush-agent4")
트리거 탭에서 사용하는 토글된 트리거와 해당 트리거를 기록합니다.
...
이 플러시 예제는 해당 디렉토리에서 .stat 파일을 업데이트하여 /content/dam 경로를 플러시합니다.
<br>.stat 파일<br>
플러싱 메커니즘은 본질적으로 단순하며 우리는 다음과 같은 중요성에 대해 설명하고자 한다. <b>.</b> 캐시 파일이 생성되는 문서 루트에서 생성되는 파일입니다.

.vhost 및 _farm.any 파일 내에서 캐시가 있는 위치와 최종 사용자의 요청이 있을 때 파일을 저장/제공할 위치를 지정하도록 문서 루트 지시문을 구성합니다.

디스패처 서버에서 다음 명령을 실행하고자 한다면 .stat 파일을 찾아야 합니다
...
![통계 수준이 표시된 컨텐츠와 날짜가 혼합된 통계 파일](https://helpx.adobe.com/content/dam/help/en/experience-manager/kb/ams-dispatcher-manual/disp-flushing/jcr_content/main-pars/image_2035486865/dispatcher-statfiles.png "dispatcher-statfiles")<br>통계 파일 레벨<br>
각 디렉터리에 .stat 파일이 있습니다.  플러시가 발생했음을 나타내는 표시기입니다.  위의 예에서 <b>통계파일레벨</b> 설정이 (으)로 설정되었습니다. <b>3</b> 해당 팜 구성 파일 내부.

통계파일레벨 설정은 .stat 파일을 통과하고 업데이트하는 폴더의 수를 나타냅니다.  .stat 파일이 비어 있으면 날짜도장이 있는 파일 이름에 지나지 않으며 수동으로 만들 수도 있지만 디스패처 서버의 명령줄에서 터치 명령을 실행할 수도 있습니다.
...
통계파일레벨을 적절한 레벨로 설정합니다.  폴더 구조를 보고 너무 많은 디렉토리를 탐색하지 않고 간결한 플러시를 허용하도록 설정되어 있는지 확인합니다.   시스템의 성능 테스트 중에 사용자의 요구 사항에 맞는지 테스트하여 확인합니다.
...
문서 루트가 /var/www/html/인 다음 번호로 설정된 /statfileslevel은 /content/dam/brand1/en/us/logo.jpg을 플러시할 때 다음과 같은 결과를 갖게 됩니다.
...
.stat 파일 레벨 0과 /var/www/html/.stat에만 stat 파일이 있는 것은 /var/www/html/content/dam/brand1/en/us/ 아래에 있는 컨텐츠가 가장 가까운 .stat 파일을 찾아 5개의 폴더를 더 탐색하여 레벨 0에 있는 유일한 .stat 파일을 찾아서 날짜와 비교합니다.  높은 수준에서 플러시하면 캐시된 모든 항목이 본질적으로 무효화됩니다.
