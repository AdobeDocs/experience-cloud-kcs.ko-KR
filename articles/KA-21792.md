---
title: "null 값에 대한 GraphQL 처리 변경 사항 | AEM"
description: 설명
...
# null 값에 대한 GraphQL 처리 변경 | AEM

## 설명 {#description}

<b>환경</b><br><br>as a Cloud Service Experience Manager<br><br> <br><br><b>문제/증상</b><br><br>릴리스 2023.02.11382에서는 <b>GraphQL</b> (오픈 소스 쿼리 언어) 구현이 이전 릴리스의 잘못된 동작에 의존할 경우 애플리케이션 코드에서 예기치 않은 동작을 초래할 수 있습니다. 
<br>아래에 언급된 해상도는 이제 AEM(Adobe Experience Manager) GraphQL이 *null* 값은 모든 필터 작업에서 일관되게 표시됩니다. <br>

## 해결 방법 {#resolution}

`CONTAINS_NOT` 및 *null* 값<br> <br>첫 번째 변경 사항은 `CONTAINS_NOT` 연산자를 사용할 수 있습니다. 이전 릴리스에서는 `CONTAINS_NOT` 에 포함된 컨텐츠 조각을 반환하지 않았습니다. *null* 필터 조건이 적용된 필드의 값입니다. 같은 유사한 연산자와 일치하지 않습니다. `EQUALS_NOT` 또는 `NOT_AT`.<br>클라이언트 응용 프로그램이 처리할 수 있는지 확인하십시오 *null* 사용 시 AEM에서 반환되는 값 `CONTAINS_NOT`.<br> <br><b>예:</b>
...
<br>모드 적용 `ALL_OR_EMPTY/ALL` 및 *null* 값<br>
2023.02.11382에서 해결된 다른 문제는 *null* 복수 값 필드의 필터 조건 값. 적용 모드 `ALL_OR_EMPTY` (배열/다중 값 필드에서)가 *null* 해당 필드의 값이 실제로 작동했으므로 이 값은 실제로 `ALL` 적용 메서드를 사용합니다. 동시에 적용 모드로 다중 값 필드 필터링 `ALL` 에는 해당 필드에 빈 컨텐츠가 있는 조각이 포함되지만 포함되면 안 됩니다. 이러한 적용 모드 중 하나를 사용하는 쿼리를 테스트하고, 예상 결과 세트를 더 이상 반환하지 않으면 이 모드를 변경하는 것이 2023.02.11382.

이 문제는 일부 연산자(2023.02.11382 이전)에만 있습니다.
예: `EQUALS_NOT` 적용 방법이 예상대로 작동했습니다. `ALL_OR_EMPTY`, `CONTAINS` 그렇지 않았습니다.
<b>예:</b>
...
<br>적용 모드: `INSTANCES`, `_instances: 0` 및 *null* 값<br>
마지막 문제는 *null* 다중 값 필드의 필터 조건에 있는 값입니다. 적용 모드 `INSTANCES` with`_instances: 0` 다음을 포함하는 컨텐츠 조각과 일치하지 않음 *null* 값은 2023.02.11382 전에 해당 필드에 있지만 2023.02.11382부터 시작됩니다. 이 성상식을 사용하는 쿼리를 테스트하고 예상 결과 집합을 더 이상 반환하지 않으면 이 쿼리를 변경하는 것이 2023.02.11382.
<b>예:</b>
...
<br>고객에게 영향을 줄 수 있는 개선 사항 `ft-sites-97` 활성화됨<br>
고객이 `ft-sites-97`*(GraphQL 페이지 매김 및 최적화된 필터링)* 기능 플래그 사용 *이전*릴리스 2023.02.11382에서 2023.02.11382으로 업그레이드한 후 다음 동작 변경 사항 중 하나 이상이 발생할 수 있습니다. 이 기능 플래그를 설정하지 않은 고객은 이 섹션의 영향을 받지 않습니다.
<br>비같음을 확인하는 조건으로 null 처리<br>
조각 필드 *null* 값이 비같음 여부를 확인하는 필터와 간헐적으로 일치하지 않았습니다.
...
<br>필터 연산자 `NOT_AT` 날짜/시간 값 포함<br>
필터 연산자 `NOT_AT` 데이터/시간 값으로 간헐적으로 제대로 작동하지 않습니다. 대신, 이 방법은 `AFTER` 조건. 
코드가 잘못된 동작을 사용하는 경우 필터 조건을 로 전환해야 합니다 `AFTER`.
<br>배열: `INSTANCES` with `_instances: 0` 제대로 작동하지 않음<br>
필터 조건 `INSTANCES` (배열/복수 값 필드에서) `_instances` 간헐적으로 0으로 설정하면 제대로 작동하지 않으며, 가 실제로 있어야 하는 어떤 것도 반환하지 않았습니다.
