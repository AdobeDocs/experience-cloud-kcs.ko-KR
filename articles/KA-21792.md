...
<b>문제/증상</b>
릴리스 2023.02.11382에서는 이전 릴리스의 잘못된 동작에 의존할 때 애플리케이션 코드에서 예기치 않은 동작을 초래할 수 있는 GraphQL(오픈 소스 쿼리 언어) 구현이 일부 개선되었습니다. 아래 언급된 해상도는 이제 Adobe Experience Manager(AEM) GraphQL이 모든 필터 작업에서 일관되게 null 값이 있는 콘텐츠를 반환하는 데 도움이 됩니다.
