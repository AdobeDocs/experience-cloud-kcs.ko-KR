...
<b>`CONTAINS_NOT` 및 *null* 값</b><br> <br>첫 번째 변경 사항은 `CONTAINS_NOT` 연산자를 사용할 수 있습니다. 이전 릴리스에서는 `CONTAINS_NOT` 에 포함된 컨텐츠 조각을 반환하지 않았습니다. *null* 필터 조건이 적용된 필드의 값입니다. 같은 유사한 연산자와 일치하지 않습니다. `EQUALS_NOT` 또는 `NOT_AT`.<br>클라이언트 응용 프로그램이 처리할 수 있는지 확인하십시오 *null* 사용 시 AEM에서 반환되는 값 `CONTAINS_NOT`.<br> <br><b>예:</b>
...
<b>고객에게 영향을 줄 수 있는 개선 사항 `ft-sites-97` 활성화됨</b>
...
<b>배열: `INSTANCES` with `_instances: 0` 제대로 작동하지 않음</b>
