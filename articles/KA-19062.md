...
의 확인 실행 모드 [!DNL oak-run] 는 세그먼트 저장소의 양호한 것으로 알려진 마지막 수정 버전을 확인하는 데 사용할 수 있습니다.  손상된 세그먼트 저장소를 수동으로 최근의 양호한 수정 버전으로 되돌리는 데 사용할 수 있습니다.
...
1. 편집을 통해 리포지토리를 이 버전으로 되돌리기 `./crx-quickstart/repository/segmentstore/journal.log` 최신 양호한 수정 버전이 포함된 라인 뒤에 있는 모든 라인을 삭제합니다.

1. 모두 제거 `./crx-quickstart/repository/segmentstore/\*.bak` 파일.
...
\*주n:\* 아래 절차는 고급 사용자를 위한 것입니다. 손상된 노드를 삭제할 때는 시스템 노드(예: `/home`, `/jcr:system`등) 또는 시스템 노드인 경우 복원할 수 있는지 확인해야 합니다.  확실하지 않은 경우 여기에 설명된 단계에 대한 도움이 필요하면 AEM 고객 지원 팀에 문의하십시오.
...
1. 를 사용하여 마지막 명령의 출력에 나열된 식별된 손상된 노드를 모두 제거합니다. `rmNodes.groovy`
...
1. 2단계에서 찾은 모든 노드에 대해 3단계를 반복합니다.

   위의 내용 `rmNode` 손상된 경로에 대해 true를 반환해야 합니다. 이는 경로를 삭제했음을 의미합니다. 해당 경로에서 rmNode 명령을 다시 실행하여 손상된 세 개의 경로가 삭제되었는지 확인하십시오. 다음에 실행하면 false를 반환해야 합니다.
   ...
   이 jar 버전은 압축 시 읽을 수 없는 바이너리를 건너뛰고 0바이트 바이너리로 바꾸고 예외 및 syserr 경로를 로깅합니다. 그런 다음 압축된 리포지토리는 [!DNL oak-run] 노드 수 스크립트를 확인하고 패치가 적용되지 않은 스크립트를 사용하여 다시 압축할 수도 있어야 합니다 [!DNL oak-run].

1. 아래를 사용하여 체크포인트를 나열하여 체크포인트 정리를 수행합니다. 체크포인트가 두 개 이상 있으면 정리합니다.
