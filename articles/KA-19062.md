...
1. rmNodes.groovy를 사용하여 마지막 명령의 출력에 나열된 식별된 손상 노드를 모두 제거합니다.
...
손상 노드 경로가 2단계에 얻은 경로인 경우 예: &quot;/content/dam/test.txt/jcr:content/renditions/original/jcr:content/&quot; 참고: oak-run.jar 버전 1.6.13 이상을 사용할 때 다음과 같은 오류가 발생하는 경우 —read-write JVM 매개 변수를 설정합니다.
...
1. 아래 체크포인트를 나열하여 체크포인트 정리를 수행합니다. 체크포인트가 두 개 이상인 경우 정리합니다.

   ```
   nohup java -Xmx4096m -jar oak-run-1.2.18.jar checkpoints /app/AEM6/author/crx-quickstart/repository/segmentstore rm-allnohup.out &
   ```
2. 오프라인 압축을 실행합니다.  오프라인 압축을 실행하는 방법을 모를 경우 다음을 참조하십시오. [여기](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. 인덱싱이 완료될 때까지 서버를 시작합니다.
