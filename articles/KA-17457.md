...
2. AEM Java 프로세스가 높은 CPU 사용률을 초래하는지 OS 수준에서 확인합니다.    
...
4. 시스템 유지 관리 절차를 검토합니다. 다음 보기 [문서](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/monitoring-and-maintaining.html)AEM 유지 관리에 대한 자세한 내용을 확인하고 다음을 포함하여 AEM에서 적절한 유지 관리를 수행하고 있는지 확인합니다. - 개정 정리 (MongoMK 및 Database DocumentNodeStore만 해당) - 매일 또는 더 자주 - Offline Tar 압축(TarMK만 해당) - Bi-weekly - Data Store 가비지 수집(FileDataStore 또는 S3 DataStore만 있는 시스템) - 주별 - 워크플로우 삭제 - 매주 - 버전 삭제 - 주별 - AuditLog 제거 - 매주 5. AEM Dispatcher 수준에서 구현된 캐싱 전략 검토.
6. 사이트의 캐싱을 검토합니다.
7. <b>감사</b> Google Chrome 브라우저의 기능 <b>개발자 도구</b> 패널.  이러한 도구는 클라이언트측 성능 개선에 대한 권장 사항을 제공합니다.
...
1. AEM 인스턴스에서 이러한 URL을 열어 비동기 색인 http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dasync%2Ctype%3DIndexStats http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dfulltext-async%2Ctype%3DIndexStats  - This URL이 AEM6.2 이상에만 적용되는 통계를 확인합니다
...
2) 효율적으로 실행되지 않는 작업 과정으로 인해 문제가 발생한 경우 동시 실행 검토 [워크플로우 처리 팁](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-process-ref.html?lang=en)
<br>TarMK 손상 문제<br>
<b>TarMK 손상 증상</b>
