...
그러나 pipelineExecutionFailureReasons는 오류 코드 &quot;002_010_006&quot; 및 메시지와 함께 &quot;ASSETS_TESTS_AUTHOR_CONNECTION_ERROR&quot;를 나타냅니다.

`Unable to connect to the author instance. `
  
내부 세부 정보에서 &quot;찾을 수 없음&quot;을 나타내는 메시지와 함께 HTTP 응답 문제를 제안합니다. 이는 일반적으로 테스트 실행 중에 작성자 인스턴스의 끝점에 연결할 수 없었음을 의미합니다.

assetsTest 단계 중에 작성자 인스턴스에 대한 연결 오류가 발생하여 테스트가 성공적으로 완료되지 않는 것이 주요 문제입니다.

그러나 cloudmanager 사용자가 존재하며 작성자에 자산을 수동으로 업로드하는 것은 예상대로 작동합니다.

에셋 성능 테스트가 30분 동안 작동하고 30분 후 테스트가 중단되는 동안 반복적인 방식으로 갑자기 실패합니다. `[` 1`]`
...
내부 Azure 함수 로그를 확인하면서 404 Not Found 결과가 있는 요청을 찾았습니다.

&quot;message&quot; 필드에서 URL에 대한 GET이 404를 반환했습니다 `[` 1`]` .

이 경우 오류는 Adobe의 권장 사항에 따라 queryBuilder가 디스패처 수준에서 차단되었기 때문에 발생했습니다.

그러나 Cloud Manager는 에셋 테스트 중에 처리 시간을 검색하므로 Cloud Manager 사용자를 위한 Dispatcher에서 QueryBuilder를 허용해야 합니다.
