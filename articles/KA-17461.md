...
2. <b>&#x200B; s-maxage </b>- <b>s-maxage </b>의 지시문 <b>Cache-Control</b> 헤더를 사용하면 CDN과 같은 공유 캐시에 대해 다른 TTL을 설정할 수 있습니다.  이 값을 설정하면 브라우저에서는 <b>최대 연령</b> 그리고 다른 캐시는 <b>s-maxage</b> 을 가리키도록 업데이트하는 것이 좋습니다.
   ...
   <b>  </b>캐시 제어: max-age=600, s-maxage=300
...
Google Chrome에서 캐싱을 테스트할 때 https를 통해 테스트하고 자체 서명된 인증서가 있는 경우 아무 것도 캐시되지 않습니다.  [신뢰할 수 없거나 잘못된 인증서가 있는 경우 Chrome이 응답을 캐시하거나 재유효성 검사를 수행하지 않습니다](https://helpx.adobe.com/experience-manager/kb/cache-problems-on-chrome-with-SSL-certificate-errors.html).
...
2. 사전 압축되지 않은 모든 파일 유형에 대한 GZip 압축 응답
   ...
   - 
      ...
      Dispatcher 수준에서 GZip 압축 활성화:

      - Apache - 사용 [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html).  mod_deflate에서 Vary를 사용하는 경우 주의하십시오.  경우에 따라 Vary 헤더에서 CDN 및 브라우저가 캐싱을 완전히 건너뛸 수 있습니다.
      - Microsoft IIS - 사용 [동적 압축](https://docs.microsoft.com/en-us/iis/configuration/system.webserver/httpcompression/).
      - 
...
3. CDN 공급자가 [엣지 사이드 포함](https://en.wikipedia.org/wiki/Edge_Side_Includes) (ESI) 이 기능을 활용합니다.

   - 


      AEM 구성 요소는 ESI를 사용하여 분리할 수 있습니다.  이렇게 하려면 Apache를 사용합니다 [Sling 동적 포함](https://github.com/Cognifide/Sling-Dynamic-Include) 또는 사용자 지정 솔루션을 구현합니다.
   - 
...
<br><br><br><br> <br><br>AEM Dispatcher 캐싱<br><br><br><br> <br><br>
CDN 캐시가 만료되면 요청이 AEM Dispatcher 캐시에 도달합니다.  이 수준에서는 캐싱을 최적화하기 위해 수행할 수 있는 작업이 많습니다.

더 큰 주제이므로 다음을 참조하십시오 [이 문서](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html) 디스패처 캐시를 최적화하는 방법에 대한 자세한 정보.
...
1. 기본적으로 AEM에서 설정하지 않은 다음 HTTP 응답 헤더를 설정합니다.
   1. <b>[캐시 제어: max-age=...](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) </b>- 이 헤더를 설정하려면 [ACS Commons - Dispatcher TTL](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-ttl/index.html) 를 사용하거나 사용자 지정 코드를 구현하여 설정할 수 있습니다.
...
2. 사이트에 개인화된/다이내믹 컨텐츠가 있는 경우:
   ...
   1. 사용 [Apache Sling Dynamic Include](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) 컨텐츠가 세분화되어 다른 시간 동안 페이지의 다른 부분을 캐시할 수 있습니다.
...
3. 클라이언트 라이브러리를 최적화합니다.
   ...
   3. [포함 클라이언트](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html)[라이브러리](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html) js 및 css 파일을 축소하고 줄이기 위해
