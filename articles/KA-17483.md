...
문서에서 변경되지 않도록 잠근 파일과 원하는 구성 설정을 올바르게 설정하는 방법을 설명합니다.<br><br> <br><br><br><br><br>왜일까요?<br><br><br><br> <br><br>
AMS가 시스템을 프로비저닝하면 모든 기능이 작동하고 안전하게 사용할 수 있는 기본 구성을 롤아웃합니다.  AMS가 기능 및 보안에 대한 기준으로서 유지하려는 것은 아닙니다.  이를 위해 일부 파일은 읽기 전용으로 표시되어 변경할 수 없습니다.

레이아웃으로 인해 해당 동작이 변경되고 필요한 변경 사항을 재정의할 수 없습니다.  이러한 파일을 변경하는 대신 원본보다 우선하는 자체 파일을 오버레이합니다.

또한 AMS가 최신 수정 사항 및 보안 개선 사항으로 디스패처를 패치할 때 파일이 변경되지는 않는다는 사실을 확인할 수 있습니다.  그런 다음 향상된 기능을 계속 활용할 수 있고 원하는 변경 사항만 적용할 수 있습니다.
![레인에서 굴러 내려오는 공이 있는 볼링 레인을 보여 줍니다.  그 공은 화살을 가지고 있고, 그 단어는 &quot;you&quot;를 나타낸다.  홈통 범퍼가 자라나고 그 위에 &quot;불변의 파일&quot;이라는 단어가 있다.](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/bowling-file-immutability.png "볼링 파일 무변형")<br><br><br><br> <br><br>
...
- 사용자 지정 변경 사항의 오버레이는 AEM의 오버레이 방식에 반영됩니다
- AMS 구성 패치는 사용자 지정 변경을 통해 수행할 수 있습니다
- 기본 설치 테스트와 사용자 지정 구성 테스트를 동시에 수행하여 문제가 사용자 지정 또는 기타 방법으로 발생하는지 확인하는 데 도움이 될 수 있습니다

<br><br><br><br>어떤 파일?<br><br><br><br> <br><br>
...
<br><br>변경 방법<br><br><br><br> <br><br>변수<br><br><br><br> <br><br>
...
| `Include /etc/httpd/conf.d/variables/ams_default.vars````IfModule` &#39;disp_apache2.c``    ``DispatcherConfig conf.dispatcher.d/dispatcher.any``    ``DispatcherLog logs/dispatcher.log``    ``DispatcherLogLevel ${DISP_LOG_LEVEL}``    ``DispatcherReceptRoot 0``    ``DispatcherUseProcessedURL 1``/``IfModule&quot; |
| --- |

<br><br><br><br><br> <br><br>
자세한 내용은 <b>DispatcherLogLevel</b> 지시문에는 표시되는 일반 값 대신 DISP_LOG_LEVEL 변수가 있습니다.  해당 코드 섹션 위에 변수 파일에 대한 include 문이 표시됩니다.  변수 파일 */etc/httpd/conf.d/variables/ams_default.vars*다음으로 우리가 보고 싶은 곳입니다.  다음은 해당 변수 파일의 내용입니다.
<br><br><br><br><br> <br><br><br><br>

| 1<br>  2개<br>  3<br>  4<br>  5개<br>  6<br>  7 | `Define DISP_LOG_LEVEL info``Define AUTHOR_WHITELIST_ENABLED 0``Define PUBLISH_WHITELIST_ENABLED 0``Define LIVECYCLE_WHITELIST_ENABLED 0``Define AUTHOR_FORCE_SSL 1``Define PUBLISH_FORCE_SSL 0``Define LIVECYCLE_FORCE_SSL 1` |
| --- | --- |

<br><br><br><br><br> <br><br>
DISP_LOG_LEVEL 변수의 현재 값이 정보에 해당하는 경우  추적하거나 디버깅하거나 선택한 숫자 값/수준을 조정하도록 조정할 수 있습니다.  이제 로그 수준을 제어하는 모든 위치가 자동으로 조정됩니다.
<br><br><br><br> <br><br>오버레이 메서드<br><br><br><br> <br><br>
자세한 내용은 [최상위 수준 포함 파일](https://helpx.adobe.com/experience-manager/kb/ams-dispatcher-manual/explanation-config-files.html#include-order) 이러한 환경은 사용자 지정의 시작점입니다.  간단한 예를 시작하기 위해 이 디스패처를 가리키려는 새 도메인 이름을 추가하려는 시나리오가 있습니다.  사용할 도메인 예는 다음과 같습니다. <b>we-retail.adobe.com. </b> 먼저 기존 구성 파일을 새 구성 파일에 복사하여 변경 사항을 추가할 수 있습니다.
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
이제 apache 웹 서버는 도메인이 의 경우 트래픽을 발생해야 한다는 것을 알지만 디스패처 모듈에 전달할 새 도메인 이름을 알려주어야 합니다.  먼저 새 \*_vhost.any 파일을 만듭니다. <b>/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any</b> 그리고 해당 파일 내에 표시할 도메인 이름을 입력합니다.
<br><br><br><br><br> <br><br><br><br>
...
| 1<br>  2개<br>  3<br>  4<br>  5개<br>  6 | `/publishfarm { ``    ``/virtualhosts {``        ``$include "/etc/httpd/conf.dispatcher.d/vhosts/ams_publish_vhosts.any"``    ``}``........SNIP.........``}` |
| --- | --- |
...
| 1<br>  2개<br>  3<br>  4<br>  5개<br>  6 | `/weretailpublishfarm { ``    ``/virtualhosts {``        ``$include "/etc/httpd/conf.dispatcher.d/vhosts/weretail_publish_vhosts.any"``    ``}``........SNIP.........``}` |
| --- | --- |
