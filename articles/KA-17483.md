...
description: "AEM에서 읽기 전용으로 표시되어 변경할 수 없는 파일을 처리하는 방법에 대해 알아봅니다."
...
이 문서에서는 잠기고 변경할 수 없는 파일과 Adobe Experience Manager에서 원하는 구성 설정을 제대로 지정하는 방법에 대해 설명합니다.
...
Adobe Managed Services(AMS)가 시스템을 프로비저닝하면 모든 것이 기능적이고 안전하도록 하는 기본 구성을 롤아웃합니다. 이러한 기능은 AMS가 기능 및 보안의 기준선으로 유지되도록 하려는 것입니다. 이를 위해 일부 파일은 읽기 전용으로 표시되어 변경할 수 없습니다.
...
변수를 사용하면 구성 파일 자체를 변경하지 않고도 기능을 변경할 수 있습니다. 변수의 값을 조정하여 구성의 특정 요소를 조정할 수 있습니다. 파일에서 강조 표시된 예 `/etc/httpd/conf.d/dispatcher_vhost.conf` 은 여기에 표시됩니다.
...
다음 방법 보기 `DispatcherLogLevel` 지시문에 다음 변수가 있습니다. `DISP_LOG_LEVEL` 일반적인 값 대신 표시되어야 합니다. 코드의 해당 섹션 위에는 변수 파일에 include 문도 표시됩니다. 변수 파일 `/etc/httpd/conf.d/variables/ams_default.vars` 다음은 를 찾는 위치입니다. 다음은 해당 변수 파일의 내용입니다.
...
위의 현재 값 을(를) 참조하십시오. `DISP_LOG_LEVEL` 변수가 &#39;info&#39;입니다. 이것을 추적하거나 디버그하거나 선택한 숫자 값/레벨로 조정할 수 있습니다. 이제 로그 수준을 제어하는 모든 곳이 자동으로 조정됩니다.

#### 오버레이 메서드

최상위 include 파일 이 모든 사용자 정의 작업의 시작이기 때문에 숙지하십시오. 간단한 예제로 시작하기 위해 이 Dispatcher를 가리키도록 새 도메인 이름을 추가하는 시나리오가 있습니다. 사용할 도메인 예는 다음과 같습니다. `we-retail.adobe.com.` 먼저 변경 사항을 추가할 수 있는 새 구성 파일에 기존 구성 파일을 복사하는 것부터 시작하십시오.
...
기존 을(를) 복사했습니다. `aem_publish.vhost` 이미 작업을 수행하는 데 필요한 것을 갖추고 있고, 이미 멋지게 시작을 했는데 다시 시작하고 싶지 않기 때문입니다.  이제 새로운 weretail.vhost 파일을 편집하고 필요한 사항을 변경합니다.
...
새 도메인 이름과 일치하도록 ServerName 및 ServerAlias를 업데이트하고 다른 이동 경로 헤더를 업데이트했습니다.  이제 Apache가 새 파일을 사용할 수 있도록 새 파일을 활성화해 보겠습니다.
...
이제 Apache 웹 서버는 도메인이 트래픽을 발생시켜야 한다는 것을 알고 있지만 여전히 Dispatcher 모듈에 적용할 새 도메인 이름이 있음을 알려야 합니다.  새 \*_vhost.any 파일 /etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any 을 생성하고, 이 파일 안에 적용할 도메인 이름을 넣으십시오.
...
이제 새 가상 호스트 항목 파일을 사용하는 새 팜 파일을 만들어야 하며 먼저 강력한 시작 파일을 새 파일에 복사합니다.
...
팜 이름과 팜 구성의 /virtualhosts 섹션에서 사용하는 include를 업데이트했습니다. 실행 중인 구성에서 사용할 수 있도록 이 새 팜 파일을 활성화해야 합니다.
