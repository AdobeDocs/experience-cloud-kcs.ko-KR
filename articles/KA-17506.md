---
title: SMPP 프로토콜 분석 [!DNL Wireshark]
description: 설명
solution: Campaign
product: Campaign
...
# SMPP 프로토콜 분석 [!DNL Wireshark]

## 설명

<br>를 사용하여 SMPP 트래픽을 분석하는 방법 알아보기 [!DNL Wireshark].<br><br><br><br><br>소개<br><br><br><br> <br><br>
...
이 문서는 다음 사양을 대체하지 않습니다. 프로토콜 사양을 해석하고 과 일치시키는 방법에 대한 유용한 팁을 제공합니다 [!DNL Wireshark] Adobe Campaign과 SMS-C 파트너 간의 문제를 해결하는 데 도움이 되도록 을(를) 표시합니다.
...
<br><br>네트워크 트래픽 캡처 [!DNL Wireshark]<br><br><br><br><br><br>
컴퓨터에 직접 액세스할 수 없는 경우 다음과 같은 명령줄 도구를 사용하여 캡처해야 할 수 있습니다 *tcpdump*. 연결의 TCP 포트를 이미 알고 있는 경우 모든 트래픽을 캡처하지 않도록 올바른 필터를 사용하십시오. 여기 샘플입니다 `tcpdump` 포트를 캡처할 명령줄 `12345` to <b>`outfile.pcap`</b>:
...
파일 <b>`outfile.pcap`</b> 그런 다음 을(를) 열 수 있습니다. [!DNL Wireshark] 추가 분석.


<br><br>[!DNL Wireshark] 처리<br><br><br><br> <br><br><br><br>
이 항목에서는 사용자가 의 기본 사항에 익숙하다고 가정합니다 [!DNL Wireshark]: 패킷 캡처, 단순 필터 정의, 패킷 세부 정보 읽기 에 대한 간단한 소개가 제공됩니다 [whotoek - 사용 방법 [!DNL Wireshark] 캡처, 필터링 및 Inspect 패킷](https://www.howtogeek.com/104278/how-to-use-wireshark-to-capture-filter-and-inspect-packets/).

에서 SMPP 트래픽을 필터링하려면 [!DNL Wireshark]에는 세 가지 중요한 기능이 있습니다.

- SMS-C의 포트에서 표시 필터를 사용합니다. 예를 들어 SMS-C가 포트를 사용하는 경우 `10000`를 채울 때는 다음 필터를 사용하십시오.
   *`tcp.port == 10000`*
...
프로토콜은 TCP를 통해 작동하며 완전히 바이너리이며, 이는 다음과 같은 특별한 도구를 의미합니다 [!DNL Wireshark] 스트림의 내용을 해독하려면 (또는 16진수 편집기가 필요합니다.)

스트림은 독립적인 PDU로 구성됩니다. 각 PDU는 명령, 상태, 시퀀스 번호 및 명령을 기반으로 하는 기타 정보를 포함하는 메시지입니다.

스트림 프로토콜로서의 TCP의 특성으로 인해 TCP 패킷은 둘 이상의 PDU를 포함할 수 있으며, PDU는 2개 이상의 TCP 패킷에 걸쳐 있을 수 있습니다. [!DNL Wireshark] PDU를 올바르게 재조합하므로 대부분 투명합니다 [!DNL Wireshark] 사용자.
...
`BIND_TRANSMITTER` 에 의해 인식됨 *`BIND_TRANSMITTER_RESP`*, *`SUBMIT_SM`* 에 의해 인식됨 *`SUBMIT_SM_RESP`*&#x200B;등

응답에 대한 시간 제한이 있는데, 일반적으로 10, 30 또는 60초입니다. 응답에는 긍정적 응답( )이 포함될 수 있습니다`command _status = 0`) 또는 오류(5.1.3 참조) *`command_status`*, *표 5-2* ( 표준 오류 목록에 대한 SMPP 사양 참조). 대부분의 경우 이러한 응답이 즉시 응답되며 응답 시간 초과에 도달하지 않습니다.
...
SMPP 연결은 TCP를 사용하여 연결하여 시작됩니다. 그런 다음 `BIND` 작업을 보낸 사람 [!DNL Campaign]에 의해 인식됨 `BIND RESP`. 이러한 작업은 SMPP 사양(`BIND` 작업).
...
in [!DNL Campaign]에 로그인하면 *`BIND_TRANSMITTER`* MT 전송을 시작할 때 및 *`BIND_RECEIVER`* 패킷 *`nlsm s`* 은(는) MO/SR 연결을 트리거합니다.

<b>송신기, 수신기 및 송수신 장치: </b>Campaign Classic용 SMPP 커넥터는 별도의 송신기/수신기 모드에서 작동합니다. 두 개의 TCP 연결이 있으며, 하나는 MT를 전송하는 연결이고, 다른 하나는 MO와 SR을 수신하는 연결입니다. TCP 연결은 항상 [!DNL Campaign]인 경우에도 마찬가지입니다.
...
수신기가 바인딩되면 SMS-C는 언제든지 MO를 전송할 수 있습니다. MO는 *`DELIVER_SM`* 비트 2-5가 포함된 PDU *`esm_clas s`* 지우기(자주) *`esm_class`* 은 단순히 0입니다.
...
다음 *`DELIVER_SM`* PDU는 *`DELIVER_SM_RESP`* 동일한 PDU *`sequence_number`*.
...
필드 *`registered_delivery`* (사양 5.2.17절에 설명)은 SMS-C에 SR이 특정 MT에 대해 요청되는지 여부를 나타냅니다. 특정 메시지에 대한 SR을 받지 않는 경우, *`SUBMIT_SM`* PDU.
...
SMS 메시지는 GSM7 인코딩이라고 하는 특수 7비트 인코딩을 사용합니다. 을(를) 참조하십시오. [!DNL Wikipedia] GSM 03.38(영어).
<br><br><br><br> <br><br>
...
- GSM7 인코딩은 [!DNL Wireshark]: 특수 문자가 잘못 표시됩니다. GSM7 문자열이 제대로 인코딩되어 있는지 확인해야 하는 경우 16진수 코드를 GSM7 테이블과 비교해야 합니다.


다음 *`data_coding`* 필드는 사용할 인코딩을 알려줍니다. 유일한 문제는 값 0이 사양에 있는 기본 SMS-C 인코딩을 의미하지만 일반적으로 GSM7을 의미한다는 것입니다. SMS-C *파트너에게 어떤 인코딩이 연결되어 있는지 확인하십시오 `data_coding = 0` (Adobe Campaign은 용 GSM7만 지원) `data_coding* = 0`).
...
[!DNL Wikipedia] 에는 사용자 데이터 헤더 및 연결된 SMS에 대한 좋은 문서가 있습니다(영어).

short_message에 UGH가 포함되어 있는지 확인하려면 esm_class의 비트 6 및 7을 확인하십시오(세부 항목의 섹션 5.2.12 참조). [!DNL Wireshark] 인터페이스에서 UGH를 구문 분석하고 정확한 정보를 제공합니다.
...
위의 스크린샷에서는 메시지 필드(1)의 사용자 데이터 헤더, UGH(2)에 포함된 정보 및 패킷에 속하지 않고 [!DNL Wireshark] (3): 짧은 메시지 본문 필드는 [!DNL Wireshark].
<br><br><br><br><br><br>수신 SR<br><br><br><br><br><br>
수신기가 바인딩되면 SMS-C는 언제든지 SR을 전송할 수 있습니다. SR은 DELIVERY_SM PDU를 사용하여 * 비트 2-5와 함께 전송됩니다.`esm_class`*설정됨.
...
다음 *`DELIVER_SM`* PDU는 *`DELIVER_SM_RESP`* 동일한 PDU *`sequence_number`*. 이 SR과 일치하는 MT를 찾으려면 *`SUBMIT_SM_RESP`* 를 반환합니다. 예를 들어 SR과 일치하는 MT입니다.
...
SR은 *`registered_delivery`* 필드는 MT에 설정되어 있습니다.
...
SMS-C 파트너에게 자체 구현에 대한 설명서를 직접 문의하고 제공된 구현과 일치하는지 다시 확인해야 합니다 [!DNL Wireshark]. 특히 SMS-C 구현자는 구현도 잘 모르고 이로 인해 문제와 오해가 발생하는 경우가 많습니다. 이 필드(특히 오류 코드)에 대한 의심이 있는 경우 SMS-C 파트너에게 도움을 요청하십시오.
...
- ID는 *에서 전송된 것과 동일합니다`SUBMIT_SM_RESP`*MT의 경우
- 의 문제를 무시할 수 있습니다 `text` 필드: 이 필드는 [!DNL Campaign] 따라서 SMS를 순수한 영숫자 ASCII가 아닌 다른 인코딩으로 전송한 경우, 이 인코딩은 유용하고 신뢰할 수 없으며 완전히 읽을 수 없습니다. 이는 정상적인 동작입니다.
- 필드 이름은 대/소문자를 구분하지 않습니다(예: `id`: `sub`: `Text`: 를 `ID`: `SUB`: `text`:)
- *`dlvrd`*필드는 SMS-C 파트너가 명시하지 않는 한 일반적으로 신뢰할 수 없습니다.
- 날짜가 시간대를 가지고 있어 사실상 쓸모가 없거나 원격 서버의 시계가 꺼져 있기 때문에 잘못된 날짜입니다.
- *`stat`*필드에는 부록 B에 정의된 값 이외의 다른 값이 있을 수 있습니다. 상식과 SMSC 파트너의 설명서를 사용하여 의미를 파악합니다.
- *`err`*필드는 SMS-C에 완전히 의존하며 SMS-C 파트너가 문서화한 대부분의 시간을 사용합니다. 종종 코드 000은 성공을 의미하지만, 다른 코드는 오류를 나타냅니다. 필드는 종종 숫자이지만 16진수일 수도 있습니다.
