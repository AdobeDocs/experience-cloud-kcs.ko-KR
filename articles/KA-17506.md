...
- SMS-C의 포트에서 표시 필터를 사용합니다.<br><br>    예를 들어 SMS-C에서 포트 10000을 사용하는 경우 다음 필터를 사용하십시오.
   `tcp.port == 10000`
...
- 를 사용하십시오 <b>TCP 스트림 따라하기</b> 작업 중인 스트림을 격리하는 도구입니다.<br><br>    SMPP와 관계가 없는 텍스트 프로토콜에만 유용하므로 표시되는 빨간색/파란색 텍스트 창을 닫습니다.
...
참고: 표준 명령 목록은 SMPP 사양(SMPP 명령 세트)의 섹션 5.1.2.1에서 찾을 수 있습니다.
...
참고: 로그인은 system_id 필드에서 찾을 수 있습니다.
...
<b>MT 인코딩</b>
주의: SMS 인코딩은 많은 트랩과 다양한 구현이 있는 복잡한 주제입니다.
...
참고: Adobe Campaign Classic SMPP 커넥터는 *SUBMIT_SM_RESP* 패킷 을 참조하십시오. 사양은 이 비헤이비어를 명시적으로 금지하지 않지만 잘못된 비헤이비어로 간주됩니다(메시지는 전송되기 전에 이미 수신됨). 이 경우가 너무 자주 발생하면 SMS-C 파트너에게 플랫폼을 수정하라고 요청합니다.
