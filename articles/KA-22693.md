---
title: "IMS 구성에 대한 클라우드 솔루션으로 Adobe Target을 선택할 수 없음"
description: 설명
...
# IMS 구성을 위한 클라우드 솔루션으로 Adobe Target을 선택할 수 없습니다.
...
고객은 AEM을 Adobe Target과 통합하기 위해 Adobe Target IMS 구성을 만들려고 합니다.

그러나 IMS 구성에 대한 클라우드 솔루션으로 &quot;Adobe Target&quot;를 선택할 때마다 &quot;인증서를 검색할 수 없음: 제공된 userId가 기존 사용자를 식별하지 않음&quot; 오류가 팝업되고 구성을 만들 수 없습니다.

이 문제는 Adobe Target 솔루션에 대해서만 발생합니다.



재현 단계:

1. 도구로 이동 `>`  보안 `>`  IMS 구성 `>`  만들기
2. Target 을 솔루션으로 선택

ui에서 &quot;인증서를 검색할 수 없습니다. 입력한 사용자 ID가 기존 사용자를 식별하지 않습니다.&quot;

로그에서 이 문제와 관련된 유일한 오류는 `[` 1`]` .
...
사용자 &quot;target-imsconfig-service&quot; 및 &quot;targetservice&quot;에 대한 권한을 확인할 때 target-imsconfig-service에 할당된 ACE가 없다는 것을 발견했습니다.
...
경로: /home/users/system/target-imsconfig-service

권한: 허용

권한: jcr:read; rep:write
