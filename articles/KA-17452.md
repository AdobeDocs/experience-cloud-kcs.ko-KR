...
<br><br><br>Linux, UNIX 또는 Windows의 JVM에서 스레드 덤프를 가져오려면 어떻게 해야 합니까?<br><br><br><br> <br><br>
...
<br><br>1단계: Java 프로세스의 PID 가져오기<br><br><br><br> <br><br>
...
<b>참고:</b> Linux 및 UNIX에서는 sudo -u 사용자 jps -l과 같이 이 명령을 실행해야 할 수 있습니다. 여기서 &quot;user&quot;는 Java 프로세스가 실행 중인 사용자의 사용자 이름입니다.
...
jstack을 사용하여 스레드 덤프를 가져오려면 다음 명령을 실행합니다. jstack -l pid

콘솔 출력 redirect/append 지시어를 사용하여 연속 스레드 덤프를 파일에 출력할 수 있습니다. jstack -l pid threadbump.log
...
- Windows에서 jstack을 실행하고 &quot;이 명령을 처리할 수 있는 저장소가 충분하지 않습니다&quot; 오류가 발생하면 jstack을 windows SYSTEM 사용자 또는 Java 프로세스를 소유하는 사용자로 실행해야 합니다.  다운로드할 수 있는 Psexec를 사용하여 이 작업을 수행할 수 있습니다 [여기](http://technet.microsoft.com/en-us/sysinternals/bb897553.aspx). jstack을 SYSTEM 사용자로 실행하려면 다음과 같은 명령을 사용합니다.
   ...
   psexec -s jstack *pid*   threadbump.log
...
- Java 프로세스가 응답하지 않는 경우 옵션을 사용하는 데 도움이 될 수 있습니다 <b>-J-d64</b> (64비트 시스템의 경우) 예를 들면 다음과 같습니다.
   ...
   jstack -J-d64-l *pid*  threadbump.log
- jstack 명령(jstack -l pid threaddump.log)에 아래 오류 1이 발생하면 java 프로세스를 소유하는 사용자로 명령을 실행합니다.  예:
...
다음과 같이 실행합니다. sudo-u *사용자* jstackSeries.sh *pid* *aemserveruser* *count* *delay*
...
1. 이 명령을 실행하여 다음을 수행합니다.
   ...
   sudo -u user kill -QUIT와 같이 이 명령을 실행해야 할 수 있습니다. *pid* 여기서 &quot;user&quot;는 Java 프로세스가 실행 중인 사용자입니다.
2. crx-quickstart/server/start 스크립트를 사용하여 CQSE를 시작하는 경우 스레드 덤프가 crx-quickstart/server/logs/startup.log으로 출력됩니다. JBoss, WebSphere, Tomcat 또는 기타 등의 타사 애플리케이션 서버를 사용하는 경우, 서버의 설명서를 참조하여 표준 출력이 지시되는 파일을 확인합니다.
...
1. Communication: -Dcom.sun.management.jmxremote를 실행하는 jvm에 다음 매개 변수를 추가합니다.
