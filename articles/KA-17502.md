...
이 문서에서는 AMS 표준 구성 파일 세트와 이 구성 표준에 대한 생각을 설명합니다.
...
AMS에서 기본 설치는 Enterprise Linux를 기본 운영 체제로 사용합니다. Apache 웹 서버를 설치하면 기본 설치 파일이 설정됩니다. 다음은에서 제공하는 기본 RPM을 설치하여 설치하는 기본 파일입니다. `yum` 리포지토리.
...
그런 다음 다음 다음 기본 디렉토리에 대한 기본값 대신 해당 볼륨을 사용합니다.
...
기존 디렉토리와 새 디렉토리가 혼동을 방지하기 위해 원래 마운트 지점에 다시 매핑된다는 점을 기억하십시오. 별도의 볼륨을 사용하는 것은 중요하지는 않지만 주목할 만한 일입니다
...
1. <b>디렉토리 재작성</b>    이 디렉터리에는 `\*_rewrite.rules` 일반 파일을 포함하는 파일을 만듭니다. *규칙 다시 작성* apache 웹 서버를 활성화하는 구문 [mod_rewrite](https://httpd.apache.org/docs/current/mod/mod_rewrite.html) 모듈.
...
Dispatcher 팜 파일 구성의 하위 섹션인 추가 조각이 있으며 이러한 파일을 분리하는 깔끔한 방법을 허용하고 일부 파일이 하나의 디렉토리에 없도록 하위 디렉토리를 만들었습니다.

1. <b>캐시 디렉터리</b>    이 디렉터리에는 `*_cache.any`, `*_invalidate.any` 모듈이 AEM에서 가져온 캐싱 요소와 무효화 규칙 구문을 처리하는 방법에 대한 규칙이 들어 있는 파일을 만듭니다.  이 섹션에 대한 자세한 내용은 [여기](https://docs.adobe.com/content/help/ko-KR/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#configuring-the-dispatcher-cache-cache)
...
4. <b>Renders 디렉터리</b>    이 디렉터리에는 `*_renders.any` dispatcher가 콘텐츠를 사용할 각 백엔드 서버에 대한 연결 세부 정보가 포함된 파일을 만듭니다.
...
Apache에 Dispatcher 모듈을 추가하는 완벽한 예를 들면 모듈을 만듭니다 `.so` 파일 위치 `/etc/httpd/modules/` 그런 다음 파일을 추가하여 포함시킵니다. `/etc/httpd/conf.modules.d/02-dispatcher.conf` 모듈을 로드할 내용 포함 `.so` 파일.
...
Apache가 제공한 기존 파일은 수정하지 않았습니다. 우리는 그들이 가야 할 디렉터리에 우리의 것을 방금 추가했습니다.
...
파일과 모듈을 추가했지만 원본 파일을 변경하지 않았다는 것을 다시 볼 수 있습니다.  이를 통해 원하는 기능을 제공할 수 있으며, 원하는 패치 수정 사항이 누락되지 않도록 보호할 수 있을 뿐만 아니라 패키지의 각 업그레이드와 최고 수준의 호환성을 유지할 수 있습니다.
