...
<br><br>기본 Enterprise Linux 폴더 구조<br><br><br><br><br><br>
...
Adobe Managed Services 서버 이미지에는 일반적으로 작은 운영 체제 루트 드라이브가 있습니다.  일반적으로 /mnt에 마운트되는 별도의 볼륨에 데이터를 배치합니다
...
- /etc/httpd/conf.d/available_vhosts/
   - 이 폴더는 .vhost라는 모든 VirtualHost / 파일을 호스팅합니다.
- /etc/httpd/conf.d/enabled_vhosts/
   - available_vhosts 폴더 내에 있는 .vhost 파일을 사용할 준비가 되면 enabled_vhosts 디렉토리에 대한 상대 경로를 사용하여 symlink로 연결합니다

<br><br><br><br> <br><br>추가적인 conf.d 디렉토리<br><br><br><br> <br><br>
Apache 구성에서 일반적인 추가 항목이 있으며, 우리는 이러한 파일을 한 디렉토리에 모두 포함하지 않고 깔끔하게 구분할 수 있도록 해주는 하위 디렉토리를 만들었습니다.

<b>Rewrites 디렉토리</b>

이 디렉토리는 일반적인 를 포함하는 \*_rewrite.rules 파일을 모두 포함할 수 있습니다. *RewriteRule* Apache 웹 서버를 사용하는 구문 [mod_rewrite](https://httpd.apache.org/docs/current/mod/mod_rewrite.html) 모듈
...
이 디렉토리는 일반적인 를 포함하는 \*_whitelist.rules 파일을 모두 포함할 수 있습니다. *IP 허용* 또는 *IP 필요* Apache 웹 서버를 사용하는 구문 [액세스 제어](https://httpd.apache.org/docs/2.4/howto/access.html)
...
이 디렉토리는 구성 파일에서 사용할 수 있는 변수가 포함된 \*.vars 파일을 모두 포함할 수 있습니다
...
<br><br><br><br> <br><br>추가적인 conf.dispatcher.d 디렉토리<br><br><br><br> <br><br>
디스패처 팜 파일 구성의 하위 섹션인 추가 항목이 있으며, 한 디렉토리에 모든 파일을 포함하지 않고 파일을 깔끔하게 구분할 수 있도록 해주는 하위 디렉토리를 만들었습니다

<b>캐시 디렉토리</b>

이 디렉토리는 모듈이 무효화 규칙 구문과 함께 AEM에서 나오는 캐싱 요소를 처리하는 방법에 대한 규칙을 포함하는 \*_cache.any, \*_invalidate.any 파일을 모두 포함합니다.  이 섹션에 대한 자세한 내용은 [여기](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#configuring-the-dispatcher-cache-cache)
...
이 디렉토리는 요청이 들어올 때 AEM에 전달할 클라이언트 헤더 목록을 포함하는 모든 \*_clientheaders.any 파일을 포함할 수 있습니다.  이 섹션에 대한 자세한 내용은 [여기](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#specifying-the-http-headers-to-pass-through-clientheaders)
...
이 디렉토리는 디스패처를 통한 트래픽이 AEM에 도달하도록 차단하거나 허용하는 모든 필터 규칙을 포함하는 모든 \*_filters.any 파일을 포함할 수 있습니다
...
이 디렉토리는 Dispatcher가 컨텐츠를 사용하는 각 백엔드 서버에 대한 연결 세부 정보를 포함하는 만드는 모든 \*_renders.any 파일을 포함할 수 있습니다.
...
이 디렉토리는 특정 팜과 특정 백엔드 서버에 일치시킬 도메인 이름 및 경로 목록을 포함하는 모든 \*_vhosts.any 파일을 포함할 수 있습니다
...
대신 원본 옆에 .rpmnew 파일을 만듭니다.  이는 원하는 일부 변경 사항을 누락하고 구성 폴더에 더 많은 가비지를 만들었다는 것을 의미합니다.

Enterprise Linux가 이 사용 사례를 더 잘 처리하도록 제대로 설정되었습니다.  기본 설정을 확장하거나 대체할 수 있는 영역을 제공합니다.  httpd의 기본 설치 내에서 /etc/httpd/conf/httpd.conf 파일을 찾을 수 있으며 다음과 같은 구문이 있습니다.
...
<br><br><br><br><br> <br><br>
아이디어는 Apache가 새 파일을 에 추가할 때 모듈과 구성을 확장하기를 원한다는 것입니다<b> /etc/httpd/conf.d/ </b>및 <b>/etc/httpd/conf.modules.d/</b> 파일 확장명이 인 디렉토리 <b>.conf</b>

디스패처 모듈을 apache에 추가할 때의 완벽한 예로서 에서는 모듈 .so 파일을 만듭니다. <b>/etc/httpd/modules/</b> 그런 다음 파일에 를 추가하여 포함시킵니다. <b>/etc/httpd/conf.modules.d/02-dispatcher.conf</b> 내용을 사용하여 모듈 .so 파일을 로드합니다.
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
<b>알림</b> apache가 이미 제공한 기존 파일을 수정하지 않았습니다.  대신, 이동하려는 디렉토리에 우리의 파일을 추가했습니다.
