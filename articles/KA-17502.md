...
이 디렉토리에는 `*_cache.any`, `*_invalidate.any` 모듈이 무효화 규칙 구문과 AEM에서 가져오는 캐싱 요소를 처리하는 방법에 대한 규칙을 포함하는 파일을 만듭니다.  이 섹션에 대한 자세한 내용은 [여기](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#configuring-the-dispatcher-cache-cache)
