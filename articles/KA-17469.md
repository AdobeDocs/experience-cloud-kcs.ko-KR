...
<br><br><br>문제: &quot;blobId에 대한 InputStream을 가져오는 동안 오류가 발생했습니다.&quot;<br><br><br><br><br><br>
다음을 수행합니다. [파일 데이터 저장소](https://docs.adobe.com/docs/en/aem/6-0/deploy/upgrade/data-store-config.html#Data%20Store%20Configurations) Adobe Experience Manager 6.x / Oak 1.x 시스템 및 `error.log` blobId용 InputStream을 가져오는 동안 오류가 발생했습니다.&quot;가 표시됩니다.
...
<br><br><br><br> <br><br>1. 최신 CFP/SP 설치<br><br><br><br> <br><br>
최신 핫픽스를 찾을 수 있습니다 [Adobe 패키지 공유](https://www.adobeaemcloud.com/content/packageshare.html).
<br><br><br><br> <br><br>2. 데이터 저장소 일관성 검사 실행<br><br><br><br> <br><br>
사용 `oak-run` jar 1.6.6 이상에서는 다음 명령을 실행합니다.
...
<br><br><br><br> <br><br>3. 누락된 파일의 경로 목록 만들기<br><br><br><br> <br><br>
누락된 파일을 복구하는 다음 단계는 누락된 파일의 경로의 전체 목록을 컴파일하는 것입니다.

1. 일관성 검사 출력에 &quot;레코드를 찾을 수 없음&quot; 오류가 발생한 모든 항목을 검색합니다.

a. <b>in [!DNL Linux] 또는 [!DNL Unix]:</b> 이 명령을 사용하여 누락된 파일 목록을 파일에 출력합니다. *missing_ds_files.txt*:
<br><br><br><br><br> <br><br><br><br>
...
\*\* &quot;blobId&quot; 오류로 인해 Adobe Experience Manager 인스턴스가 시작되지 않는 경우에는 아래에서 로그 파일을 검색합니다. `crx-quickstart/logs` &quot;blobId에 대한 InputStream을 가져오는 동안 오류가 발생했습니다.&quot; 오류가 발생하는 모든 경우:
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
명령이 제대로 작동하면 `missing_ds_files.txt` 의 모습은 다음과 비슷합니다.
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
나. <b>in [!DNL Windows]:</b>  다음과 같은 텍스트 편집기 사용 [!DNL Textpad] 또는 [!DNL Notepad++] &quot;레코드를 찾을 수 없음&quot;의 모든 항목을 찾습니다.  그런 다음 매크로를 사용하여 파일 이름을 추출하거나 스크립트를 작성하거나 새 텍스트 파일에서 파일 이름을 수동으로 복사하여 구성합니다.
...
<br><br><br><br><br> <br><br>
레코드 id는 `add1dd8fc5093b27b1fae1b753cb48b24ef3231f` 그리고 파일 경로는 다음과 같습니다. `ad*/d1/dd/add1dd8fc5093b27b1fae1b753cb48b24ef3231f*`
<br><br><br><br> <br><br>4. 누락된 파일 복구<br><br><br><br> <br><br>
...
in [!DNL Linux]작업 중인 각 Adobe Experience Manager 인스턴스에 로그인하고 resync와 같은 명령을 사용하여 인스턴스에 있는 누락된 파일을 복사할 수 있습니다.  예를 들어 누락된 파일이 있는 서버의 데이터 저장소 디렉토리에서 다음을 실행합니다.
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
명령은 다음 파일에 나열된 파일을 복사하는 rsync를 실행합니다 `missing_ds_files.txt` 서버에 있습니다.
<br><br><br><br> <br><br>5. 복구할 수 없는 파일 참조 정리<br><br><br><br> <br><br>
...
누락된 데이터 저장소 파일을 참조하는 나열된 각 노드 경로를 검토합니다.  사용자와 함께 페이지에 업로드된 DAM 자산 또는 누락된 파일을 검토합니다.  필요한 누락된 항목을 다시 업로드하도록 합니다.  필요하지 않은 모든 사용자는 Adobe Experience Manager 사용자 인터페이스를 통해 안전하게 삭제할 수 있습니다.  누락된 항목이 있으면 `/var/audit` 또는 `/var/eventing` 안전하게 삭제할 수 있습니다.  확실하지 않은 파일의 경우 [여기](https://helpx.adobe.com/kr/marketing-cloud/contact-support.html) 도움이 필요하면 AEM 지원 팀에 문의하십시오.
<br><br><br><br> <br><br>6. 모든 비동기 Oak 인덱스 다시 인덱스<br><br><br><br> <br><br>
인덱스 파일은 Oak 저장소 내에 저장되므로 일부 파일이 `/oak:index` lucene 기반 인덱스.

누락된 파일은 `/system/console/repositorycheck` 저장소 순서에서 숨겨지므로 일관성 검사  안타깝게도 이러한 파일은 다른 AEM 인스턴스와 일치하지 않습니다.
...
1. 다운로드 `oak-run` Oak 환경에 설치한 버전과 일치하는 1.0.x: [https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/](https://repo1.maven.org/maven2/org/apache/jackrabbit/oak-run/)

2. 업로드 `oak-run` Adobe Experience Manager 서버에 연결할 수도 있습니다.
...
<br><br><br><br><br> <br><br>
5. Adobe Experience Manager을 다시 시작하고 로그 파일에서 INFO 로그 메시지를 확인합니다. `org.apache.jackrabbit.oak.plugins.index`.  색인에 대한 자세한 내용을 보려면 다음 위치로 이동하십시오. `/system/console/slinglog` UI에 대한 디버그 수준 로깅 활성화 `org.apache.jackrabbit.oak.plugins.index`.
