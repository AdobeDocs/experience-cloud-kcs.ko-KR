---
title: "Campaign에 Audience Manager"
description: 설명
...
2. CRM 데이터베이스를 Campaign 고객과 동기화하려면 CRM 데이터를 Campaign에 푸시하는 프로세스를 설정해야 합니다. 전자 메일, CRM_ID는 동기화할 최소 필드입니다.
...
4. DTM은 &quot;marketing cloud ID service&quot; 도구의 CRM_ID 구성을 읽습니다. hashed_email 데이터 요소는 dataLayer에서 사용할 수 있는 CRM_ID를 읽는 DTM에 구성됩니다.
...
Campaign이 AAM에서 세그먼트를 푸시하고 가져올 수 있도록 AAM용으로 만들어진 sFTP를 Adobe Campaign으로 구성해야 합니다. sFTP 자격 증명은 Adobe 고객 지원 센터 또는 Adobe 담당자가 제공합니다. 제공된 동안 ftp-in 및 ftp-out 위치는 모두 Campaign에서 AAM에서 Campaign으로 대상을 전송하고 세그먼트를 온보딩하는 데 사용되는 프로비저닝입니다.
...
6. &#x200B;&#x200B;&#x200B;&#x200B;&#x200B;&#x200B;&#x200B;Datasource와 AAMDatasource DestinationsDatasource Destinations를
...
7. &#x200B;&#x200B;&#x200B; Adobe Campaign 내에서 데이터 소스 구성 그러면 데이터 소스가 Adobe Campaign에 구성됩니다. Adobe Campaign UI에 두 개의 데이터 소스가 만들어집니다. 하나는 선언된 ID 또는 해시된 고객 ID와 marketing cloud id 또는 요청자 UUID를 기반으로 하는 것입니다. 각 ID에 해당하는 대상 ID가 각 필드에 추가됩니다. 예를 들어 AAM 대상 Adobe Campaign - 선언된 ID에 대한 대상 ID가 조정 키가 해시된 고객 ID인 데이터 소스 설정에 추가됩니다.
...
8. 사이트에서 사용자 동작 추적 아래 사용 사례에서는 고객이 사용하는 검색 키워드가 evar2에서 캡처되고 Analytics에서 제품 구매를 구매 이벤트로 캡처합니다. 아래 흐름은 AAM에 대한 AA 호출의 서버측 전달을 기반으로 합니다. 검색 페이지에서 &quot;q&quot; 매개 변수를 캡처하여 데이터 요소에 넣습니다.
...
그런 다음 감사 인사 페이지의 analytics에서 &quot;구매&quot; 이벤트도 캡처합니다.
...
이때 고객을 추적하고 Audience Manager에서 세그먼트를 만들 수 있습니다. 또한 Audience Manager과 Campaign 모두 동일한 CRM_ID로 사용자를 수집하고 있습니다.

9. &#x200B;&#x200B; 사용자 행동&#x200B;을 기반으로 AAM에서 세그먼트를 만든 후 다음 트레이트를 만듭니다. &quot;겨울 추수&quot;:
...
10. &#x200B;&#x200B; 캠페인&#x200B;으로 세그먼트&#x200B;을 푸시 AMC 대상을 campaign과 공유하려면 세그먼트가 AAM에 만들어지면 Adobe Campaign 내에서 구성을 수행해야 합니다. 먼저 Adobe Campaign 내에서 공유 대상을 만들고 세그먼트 목록에서 AMC 세그먼트를 선택합니다. 두 공유 대상은 MID를 기반으로 만들고 캠페인 내에서 선언된 ID를 기반으로 만들어야 합니다. 구성이 완료되면 세그먼트를 AAM 내의 각 대상에 자동으로 매핑된 것으로 볼 수 있습니다.
...
11. AMC를 통해 AAM과 캠페인 대상자 공유 AMC를 통해 AAM과 캠페인 대상자를 공유하려면 Adobe Campaign 내에서 다음 구성이 필요합니다. Campaign UI의 관리 메뉴 옵션에서 프로덕션 기술 워크플로우를 선택합니다. 새로운 기술 워크플로우를 클릭하고 쿼리를 드래그한 다음 공유 대상자를 업데이트한 다음 끝납니다.
...
campaign에서 먼저 대상을 기반으로 목록을 만듭니다. 목록:
...
이제 팝업을 통해 Marketing Cloud 대상자를 선택할 수 있습니다. &quot;search_for_winter_did_not-buy&quot;를 선택합니다.
...
Adobe ID을 사용하여 campaign에 로그인하지 않았을 수 있습니다. Adobe ID을 사용하여 로그인한 경우 Marketing Cloud Enterprise Console UI의 Audience Library 제품에 추가되었는지 확인하십시오.
...
3. AAM UI에서 캠페인 대상에 세그먼트를 매핑하지 &#x200B; 마십시오
