...
<br><br>개요<br><br><br>주의<br><br><br>
이 문서는 일반적인 예제 안내서로만 제공됩니다. 시작하기 전에 Adobe Campaign Customer Success Manager와 협력하여 배포의 정확한 크기 조정을 측정해야 합니다 [!DNL Campaign] 프로젝트. <b>금지</b> 이 작업이 완료될 때까지 모든 인프라 또는 하드웨어를 구입하거나 배포합니다.

이 문서에서는 온-프레미스 데이터 센터 또는 가상화 클라우드 환경에서 Adobe Campaign Classic v7 배포에 대한 일반적인 권장 사항을 제공합니다. 이러한 유형의 배포라고 합니다 <b>하이브리드</b> 또는 <b>중간 소싱</b>, 위치 [!DNL Campaign] Adobe 클라우드 메시징 서비스를 사용하여 이메일, SMS 또는 SMPP 메시지를 보내고 이메일 열기, 바운스, 클릭 추적 데이터를 수집하는 동안 운영 제어에서 마케팅 인스턴스 및 마케팅 데이터베이스를 사용합니다.
...
- 이메일 추적 데이터는 [!DNL Campaign] 90일 시스템
...
일반 지침<br><br>
[!DNL Campaign] 데이터베이스 중심 애플리케이션이며 데이터베이스 서버 성능이 매우 중요합니다. 워크플로우, 세그멘테이션, 추적 데이터 업로드, 인바운드 상호 작용, 분석 및 기타 활동은 모두 데이터베이스 활동을 생성합니다. 일반적으로 이러한 작업의 크기와 빈도는 데이터베이스 서버의 크기를 결정합니다.

마케팅 인스턴스의 애플리케이션 서버에는 워크플로우를 실행하고 의 요청을 포함하여 SOAP API 호출에 응답할 충분한 CPU 및 메모리가 필요합니다 [!DNL Campaign] 콘솔 사용자. CPU 요구 사항은 복잡한 오퍼 규칙, 사용자 지정 JavaScript를 실행하는 워크플로우 및 트래픽 수준이 높은 웹 애플리케이션을 사용하여 아웃바운드 상호 작용을 사용하는 워크플로우에서 중요할 수 있습니다.

[!DNL Campaign] 웹 응용 프로그램을 마케팅 인스턴스 앱 서버 또는 별도의 웹 서버 시스템에 배포할 수도 있습니다. 웹 애플리케이션 작업 로드가 중요한 워크플로우와 충돌하므로 [!DNL Campaign] 콘솔 사용자, 웹 애플리케이션 및 인바운드 상호 작용이 코어 [!DNL Campaign] 우수한 성능을 통해 안정적으로 기능을 실행할 수 있습니다.
...
<br><br>역방향 프록시<br><br>
다음 [!DNL Campaign] 아키텍처는 HTTP(HTTPS)를 통해 SSL을 사용하여 마케팅 인스턴스와 Adobe 클라우드 메시징 간에 통신함으로써 높은 보안을 적용합니다. DMZ(Demilitarized Zone) 서브넷에서 역방향 프록시를 사용하여 마케팅 인스턴스 서버와 데이터베이스를 분리하고 보안을 강화함으로써 보안, 안정성 및 가용성이 강제 적용됩니다.
<br><br>로드 밸런서<br><br>
...
이러한 시스템은 마케팅 인스턴스 Application Server를 만들어 [!DNL Campaign] 사용자를 콘솔하고 캠페인 워크플로우를 실행합니다.
...
구독 옵트인/옵트아웃 및 기본 설정 센터 기능은 [!DNL Campaign] 또는 자체 웹 사이트에 의해서도 가능합니다. 웹 사이트에서 이 기능을 구현하도록 선택하는 경우, 기본 설정 및 구독 정보가 [!DNL Campaign] 마케팅 데이터베이스에 포함될 수도 있습니다. 일반적으로 다음을 통해 업로드되는 추출 파일을 만들어 수행합니다 [!DNL Campaign] 워크플로우는 자동으로 생성됩니다.
...
애플리케이션 서버가 직접 지원 [!DNL Campaign] 사용자 콘솔과 캠페인 워크플로우 실행. 이 기능은 동일한 두 서버에 구축되어 고가용성을 보장하며 NAS(Network-Attached Storage) 파일 시스템을 공유하여 페일오버를 가능하게 합니다.

웹 서버 호스트 [!DNL Campaign] 시스템에서 1천만 명의 활성 수신자를 지원하는 웹 애플리케이션입니다.
...
5천만 명의 수신자를 지원하는 배포는 기본적으로 과 동일합니다. [시나리오 2](https://experienceleague.adobe.com/docs/campaign-classic/using/technotes/hardware-sizing.html?lang=en#scenario-2): [!DNL Campaign] 웹 응용 프로그램 트래픽이 [!DNL Campaign] 웹 서버이므로 대규모 캠페인 시작 후 웹 트래픽의 발생은 영향을 주지 않습니다 [!DNL Campaign] 워크플로우 및 클라이언트 콘솔 사용자.
...
- <b>SMS 메시지 볼륨</b>
   ...
   이메일 캠페인 크기와 마찬가지로 SMS 메시지 볼륨도 큰 로드를 배치하지 않습니다 [!DNL Campaign] 온-프레미스 서버; 대부분 클라우드의 Adobe 클라우드 메시징 서버에 로드됩니다. 이메일 및 DM과 같은 SMS 캠페인에 대한 세그멘테이션은 마케팅 데이터베이스에 상당한 로드를 배치할 수 있습니다. 따라서 SMS 캠페인 실행 빈도와 세그멘테이션의 복잡성은 SMS 메시지 볼륨보다 더 관련이 있습니다.
- <b>데이터베이스 스키마 복잡성</b>
...
- <b>인바운드 상호 작용 또는 SOAP API 사용</b>
   ...
   인바운드 상호 작용 규칙 및 오퍼는 마케팅 데이터베이스에서 평가되므로 상당한 데이터베이스 서버 리소스(특히 CPU)가 필요합니다. 인바운드 상호 작용 또는 SOAP API를 많이 사용하려면 작업 로드를 실행 중인지 구분하기 위해 별도의 웹 서버가 필요합니다 [!DNL Campaign] 워크플로우.
- <b>추적 데이터 유지 기간</b>
...
<br><br>가상화<br><br>
모두 [!DNL Campaign] 서버는 가상화를 위한 좋은 후보입니다. 적절한 가용성과 성능을 보장하려면 몇 가지 문제가 해결되어야 합니다.
